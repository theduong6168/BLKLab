...
![Arduino Uno R3](https://www.arduino.cc/en/uploads/Main/Arduino_Uno_R3.jpg)
...
![Arduino Nano](https://www.arduino.cc/en/uploads/Main/Arduino_Nano.jpg)

Arduino Nano là phiên bản thu gọn của Uno, sử dụng vi điều khiển ATmega328 (Nano 3.x) hoặc ATmega168 (Nano 2.x), phù hợp cho các hệ thống nhỏ gọn.

**Điểm khác biệt so với Uno R3:**
- Sử dụng cổng Mini-B USB thay vì USB chuẩn.
- Thêm 2 chân Analog.
- Không có jack nguồn DC.

## 3. Arduino Leonardo

![Arduino Leonardo](https://www.arduino.cc/en/uploads/Main/Arduino_Leonardo.jpg)

Arduino Leonardo sử dụng vi điều khiển ATmega32u4, tích hợp chip USB-to-Serial.

### Điểm khác biệt so với Uno R3:
- Tích hợp chip USB-to-Serial trong ATmega32u4, không cần chip phụ.
- Có thể giả lập chuột, bàn phím, joystick.
- Giá thành: €18.00 (theo www.arduino.cc).
- 20 chân Digital I/O (7 chân PWM).
- 12 chân Analog (các chân PWM có thể dùng như Analog).

## 4. Arduino Mega 2560

![Arduino Mega 2560](https://www.arduino.cc/en/uploads/Main/Arduino_Mega2560.jpg)

Arduino Mega 2560 sử dụng vi điều khiển ATmega2560, là phiên bản nâng cấp của Uno, phù hợp cho các dự án phức tạp như máy in 3D, robot.
...
Dưới đây là phần giới thiệu về Arduino UNO R4 được thêm vào nội dung Markdown trước đó, cùng với bảng thông số kỹ thuật của Arduino Uno R3 đã được chuyển đổi thành bảng. Tôi đã tích hợp phần giới thiệu về Arduino UNO R4 (bao gồm cả hai phiên bản Minima và WiFi) dựa trên thông tin mới nhất từ các nguồn đáng tin cậy, đồng thời giữ nguyên cấu trúc của nội dung bạn cung cấp trước đó. Phần giới thiệu về Arduino UNO R4 sẽ được thêm vào mục "Các Board Mạch Arduino Trên Thị Trường" để đảm bảo tính mạch lạc.
Nội dung Markdown hoàn chỉnh với phần Arduino UNO R4
markdown# Các Board Mạch Arduino Trên Thị Trường
...
Dưới đây là phần mô tả được viết lại cho Arduino Mega 2560, giữ nguyên định dạng và phong cách của các phần mô tả trước đó, đồng thời đảm bảo nội dung rõ ràng, chi tiết và chuyên nghiệp. Phần này sẽ được tích hợp vào nội dung Markdown hiện có, thay thế phần mô tả cũ của Arduino Mega 2560. Tôi cũng sẽ giữ cấu trúc phù hợp để thêm vào file README.md trên GitHub.
Nội dung Markdown với mô tả Arduino Mega 2560 được viết lại
markdown# Các Board Mạch Arduino Trên Thị Trường
...
![Arduino UNO R4 WiFi](https://www.arduino.cc/en/uploads/Main/Arduino_UNO_R4_WiFi.jpg)
